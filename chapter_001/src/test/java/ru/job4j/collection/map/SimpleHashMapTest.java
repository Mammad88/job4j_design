...
public class SimpleHashMapTest {
    ...
    @Before
    public void init() {
        users.insert(0, new User("Alex", 4, this.currentTime));
        users.insert(1, new User("Micky", 10, this.currentTime));
        users.insert(2, new User("Pavel", 15, this.currentTime));
    }

    @Test
    public void whenInsertedThenGetReturnsCorrectUsers() {
        assertThat(users.get(0).getName(), is("Alex"));
        assertThat(users.get(1).getChildren(), is(10));
        assertThat(users.get(2).getBirthday().getTimeInMillis(), is(this.currentTime.getTimeInMillis()));
    }
